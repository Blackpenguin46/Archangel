...
╠══════════════════════════════════════════════════════════════════════════════╣
║ Total AI Decisions: {analysis['total_decisions']}
║ Learning Events: {analysis['total_learning_events']}
║ Learning Velocity: {analysis['learning_velocity']:.2f} events/hour
║
║ 🔴 RED TEAM PERFORMANCE:
║   • Decisions Made: {analysis['red_team_adaptation']['total_decisions']}
║   • Adaptations: {analysis['red_team_adaptation']['total_adaptations']}
║   • Adaptation Rate: {analysis['red_team_adaptation']['adaptation_rate']:.2%}
║   • Avg Confidence: {analysis['red_team_adaptation']['average_confidence']:.2f}
║   • Strategy Diversity: {analysis['red_team_adaptation']['strategy_diversity']:.2f}
║
║ 🔵 BLUE TEAM PERFORMANCE:
║   • Decisions Made: {analysis['blue_team_adaptation']['total_decisions']}
║   • Adaptations: {analysis['blue_team_adaptation']['total_adaptations']}
║   • Adaptation Rate: {analysis['blue_team_adaptation']['adaptation_rate']:.2%}
║   • Avg Confidence: {analysis['blue_team_adaptation']['average_confidence']:.2f}
║   • Strategy Diversity: {analysis['blue_team_adaptation']['strategy_diversity']:.2f}
║
║ 🧠 CROSS-TEAM LEARNING:
║   • Cross-Learning Events: {analysis['cross_team_learning']['cross_learning_events']}
║   • Competitive Evolution: {analysis['cross_team_learning']['competitive_evolution']}
║
║ ⚡ EMERGENT STRATEGIES:
"""
        
        for strategy in analysis['emergent_strategies']:
            report += f"║   • {strategy['pattern']} (x{strategy['frequency']})\n"
...
"""
    ...
    def _store_ai_decision_db(self, decision: AIDecision):
        ...
        """, (
            ...
            json.dumps(decision.input_state), json.dumps(decision.output_action),
            decision.timestamp.isoformat(), decision.execution_success,
            json.dumps(decision.learned_from)
        ))
        conn.commit()
        conn.close()
    ...
    def _store_system_event_db(self, event: SystemEvent):
        ...
        """, (
            ...
            json.dumps(event.affected_systems), event.severity,
